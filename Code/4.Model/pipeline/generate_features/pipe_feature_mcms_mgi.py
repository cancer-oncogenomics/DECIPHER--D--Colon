...
def generate_feature_mcms_mgi(f_bam, f_summary, ds_level, sample_id, f_mcms, f_mc=None, d_tmp=None, threads=5):
    """ MC和MCMS特征生成命令
    ...
    cmd1 = f"/dssg/home/hep/.conda/envs/pointy/bin/snakemake " \
           f"--snakefile {d_soft}/MCMS-MGI/scripts/mutsig.smk " \
           f"-d {d_tmp} --config " \
           f"WD={d_soft}/MCMS-MGI " \
           f"SID={sample_id} " \
           f"BAM={f_bam} " \
           f"BAI={f_bam}.bai " \
           f"QCS={f_summary} " \
